# COTK PLAY - one-click playable: PS3 client -> config -> servers -> game.
#
...
if (-not $env:COTK_API_URL) { $env:COTK_API_URL = 'http://localhost:8080' }
if (-not $env:COTK_GAME_SERVER) { $env:COTK_GAME_SERVER = '127.0.0.1:20042' }
...
function Info($m) { Write-Host "[COTK] $m" -ForegroundColor Cyan }
...
Write-Host ""
Write-Host "=== COTK : H1Z1 KotK Pre-Season 3 (build 0.23.4.161178) ===" -ForegroundColor Magenta
