...
internal sealed class MainForm : Form
{
    ...
    private void BuildUi()
    {
        ...
        loginCard.Controls.AddRange(new Control[]
        {
            ...
            _status, _welcome, _play, _logout, cardRule, safety,
        });
    ...
    private async Task<bool> CheckUpdatesAsync()
    {
        ...
        try
        {
            var result = await _updates.EnsureAsync(
                new Progress<string>(message => SetStatus(message, Theme.InkDim)),
                _closing.Token);
            _updatesChecked = true;
            _updateError = null;
            if (result.ClientVersion is not null)
                _versionLabel.Text = $"CLIENT PS3  •  BUILD {result.ClientVersion}  •  LAUNCHER {UpdateService.CurrentLauncherVersion()}";
            GameLauncher.Log($"updates: {result.Message}");
            if (result.RestartRequired)
            {
                SetStatus(result.Message, Theme.Warn);
                BeginInvoke(Close);
                return false;
            }
            return true;
        ...
        {
            GameLauncher.Log("updates: client download declined by user");
            SetStatus("Téléchargement du client annulé. Il sera proposé au prochain clic sur JOUER.", Theme.Warn);
            return true;
        }
        catch (UpdateException ex)
        {
            _updateError = ex.Message;
            GameLauncher.Log($"updates failed: {ex.Message}");
            SetStatus("Mise à jour obligatoire impossible. Consultez launcher\\data\\launcher.log.", Theme.Red400);
            return true;
        }
        catch (OperationCanceledException) when (_closing.IsCancellationRequested)
        {
    ...
    private async Task PlayAsync()
    {
        ...
        try
        {
            if (!_updatesChecked && !await CheckUpdatesAsync()) return;
            if (_updateError is not null)
            {
                SetStatus("Mise à jour obligatoire non terminée.", Theme.Red400);
                return;
            }
            ...
            for (var attempt = 1; attempt <= maxAttempts; attempt++)
            {
                ...
                var result = await GameLauncher.MonitorGameAsync(
                    ...
                    attempt,
                    operationId,
                    stage => ShowClientStage(stage, attempt, maxAttempts),
                    _closing.Token);
    ...
    private void ShowSignedIn(LauncherSession session)
    {
        ...
        SetStatus("Compte vérifié. Prêt à jouer.", Theme.Warn);
        RefreshGameControls();
    }
    ...
    private static async Task FetchPatchNotesAsync(RichTextBox box)
    {
        try
        {
            ...
            var notes = await http.GetFromJsonAsync<List<PatchNote>>(
    ...
    private static void FillFallbackNews(RichTextBox box)
    {
        ...
        Head("PATCH v1.2 - 24 AOÛT 2026");
        Item("• LAUNCHER : authentification sécurisée via le site COTK.");
        Item("• SESSION : stockage protégé par le Gestionnaire d'identification Windows.");
        Dim("  - ÉQUIPE COTK");
        Head("PATCH v1.1 - 24 AOÛT 2026");
        Item("• RÉSEAU : refonte du flux de chargement du monde.");
        Item("• ADMIN : console F8 réservée au rôle administrateur du site.");
        Dim("  - ÉQUIPE COTK");
        Head("PATCH v1.0 - 23 AOÛT 2026");
        Item("• Premier match Battle Royale solo jouable sur le build PS3 authentique.");
        Item("• LOOT, COMBAT, VÉHICULES, PERSONNALISATION.");
        Dim("  - UNE COURONNE À LA FOIS.");
        box.SelectionStart = 0;
        box.SelectionLength = 0;
        box.ScrollToCaret();
    }
