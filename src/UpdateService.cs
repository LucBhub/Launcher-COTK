...
_apiUrl = LauncherConfig.ApiUrl.TrimEnd('/');
    ...
    internal static bool IsClientInstalled(ClientRelease release) =>
        File.Exists(Path.Combine(GameLauncher.ClientDir, release.Executable));
    ...
    internal async Task EnsureClientAsync(ClientRelease release, IProgress<string>? progress, CancellationToken cancellationToken)
    {
        ...
        if (!File.Exists(executable))
        {
            // Premiere installation : ou installer le client ?
            if (PickClientFolder is not null)
            {
        ...
        CancellationToken cancellationToken)
    {
        ...
        while ((read = await input.ReadAsync(buffer, cancellationToken)) > 0)
        {
            ...
            progress?.Report($"TÃ©lÃ©chargement {name}: {(expectedSize > 0 ? total * 100 / expectedSize : 0)} %");
        }
        ...
        {
            File.Delete(path);
            throw new UpdateException($"VÃ©rification SHA-256 Ã©chouÃ©e pour l'archive {name}.");
        }
        return path;
    }
    ...
    private static async Task InstallClientArchiveAsync(string archive, ClientRelease release, IProgress<string>? progress, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (!File.Exists(stagedExe))
                throw new UpdateException($"L'archive ne contient pas {release.Executable} Ã  sa racine.");
