...
_apiUrl = LauncherConfig.ApiUrl.TrimEnd('/');
    ...
    internal async Task EnsureClientAsync(ClientRelease release, IProgress<string>? progress, CancellationToken cancellationToken)
    {
        ...
        var downloadDir = Path.GetDirectoryName(clientDir);
        if (string.IsNullOrWhiteSpace(downloadDir)) downloadDir = clientDir;
        var archive = await DownloadVerifiedAsync(release.ArchiveUrl, release.Sha256, release.SizeBytes, "client", progress, cancellationToken, downloadDir);
    ...
    {
        ValidateDownloadUrl(url);
        var dir = downloadDir ?? Path.GetTempPath();
        if (!string.IsNullOrWhiteSpace(downloadDir)) Directory.CreateDirectory(dir);
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            // Le serveur n'a pas autant d'octets que notre .part : l'archive
            // distante a change depuis la tentative precedente. Sans ce
            // nettoyage, chaque reprise repartait sur le meme Range invalide
            // et bouclait en 416 jusqu'au bout des 6 tentatives.
            if (offset > 0)
            {
                GameLauncher.Log($"download {name}: 416 avec un .part de {offset} octets, archive distante plus courte : reprise a zero.");
                File.Delete(path);
                throw new HttpRequestException("Telechargement partiel obsolete (416), reprise a zero.");
            }
            return;
        }
        ...
        if (response.Content.Headers.ContentLength is long length
            ...
            throw new UpdateException(
                $"Taille inattendue pour l'archive {name} : {(offset + length) / 1024 / 1024} Mo servis au lieu de {expectedSize / 1024 / 1024} Mo. "
                + "L'archive publiee ne correspond pas au manifeste ; reessaie plus tard.");
    ...
    {
        // .zip = archive historique ; .part = archive telechargee avec reprise
        // (renommee .zip n'existe plus : le telechargement reprise garde
        // l'extension .part jusqu'a la verification SHA-256).
        var extension = Path.GetExtension(path);
        var extensionOk = string.Equals(extension, ".zip", StringComparison.OrdinalIgnoreCase)
            || string.Equals(extension, ".part", StringComparison.OrdinalIgnoreCase);
        if (!Path.IsPathRooted(path) || !File.Exists(path) || !extensionOk)
