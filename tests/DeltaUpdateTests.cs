...
public sealed class DeltaUpdateTests : IDisposable
{
    ...
    // ------------------------------------------------------------------
    // 5. Regression : l'updater doit accepter une archive .part (telechargement
    //    avec reprise). Avant le fix, il rejetait tout sauf .zip et la mise a
    //    jour mourait silencieusement ("Archive locale invalide").
    // ------------------------------------------------------------------
    [Fact]
    public async Task ApplyLauncherUpdate_AcceptsPartArchive()
    {
        var stage = Path.Combine(_sandbox, "update-stage");
        Directory.CreateDirectory(stage);
        var zipBytes = ZipOf(("COTK.Launcher.exe", "contenu-0.10.1"));
        var archive = Path.Combine(Path.GetTempPath(), $"cotk-launcher-{Guid.NewGuid():N}.part");
        File.WriteAllBytes(archive, zipBytes);

        var target = Path.Combine(_sandbox, "install");
        Directory.CreateDirectory(target);
        File.WriteAllText(Path.Combine(target, "COTK.Launcher.exe"), "ancien");

        // Un PID deja termine : WaitForExitAsync l'ignore (ArgumentException).
        var dead = Process.Start(new ProcessStartInfo("cmd.exe", "/c exit") { CreateNoWindow = true, UseShellExecute = false })!;
        await dead.WaitForExitAsync();

        var planPath = Path.Combine(Path.GetTempPath(), $"cotk-test-plan-{Guid.NewGuid():N}.json");
        var plan = new
        {
            archivePath = archive,
            targetDirectory = target,
            launcherExecutable = Path.Combine(target, "COTK.Launcher.exe"),
            parentProcessId = dead.Id,
            updaterExecutable = Path.Combine(stage, "updater.exe"),
        };
        File.WriteAllText(planPath, JsonSerializer.Serialize(plan));
        try
        {
            await UpdateService.ApplyLauncherUpdateAsync(planPath);
            Assert.Equal("contenu-0.10.1", File.ReadAllText(Path.Combine(target, "COTK.Launcher.exe")));
        }
        finally
        {
            File.Delete(archive);
    ...
    // ------------------------------------------------------------------
    // 6. Regression : un settings.json avec BOM (ecrit par PowerShell) ne
    //    doit pas reinitialiser silencieusement le dossier client.
    // ------------------------------------------------------------------
    [Fact]
    public void LauncherSettings_Load_ToleratesBom()
    {
        var settingsPath = Path.Combine(
            Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "COTK", "settings.json");
        var original = File.Exists(settingsPath) ? File.ReadAllBytes(settingsPath) : null;
        try
        {
            Directory.CreateDirectory(Path.GetDirectoryName(settingsPath)!);
            var bom = new byte[] { 0xEF, 0xBB, 0xBF };
            var body = Encoding.UTF8.GetBytes("{\"clientDir\": \"" + _clientDir.Replace('\\', '/') + "\"}");
            File.WriteAllBytes(settingsPath, bom.Concat(body).ToArray());

            LauncherSettings.Load();

            Assert.Equal(_clientDir, LauncherSettings.ClientDirectory);
        }
        finally
        {
            if (original is not null) File.WriteAllBytes(settingsPath, original);
        }
    }
